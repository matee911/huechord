...
The minimum supported Photoshop version is `host[0].minVersion` in `uxp.config.ts` — a setting independent of the manifest version, which only bounds what is possible. [ADR-006](006-manifest-v6.md) folded the floor into its own decision, which conflated the two; this ADR separates them.
...
- Raising the floor again is a change to this decision, so it warrants a new ADR rather than an edit here — see the header-field rules in [CONTRIBUTING.md](../../CONTRIBUTING.md).
